...
public class Game {
	...
	public void handleActions(String c){
		switch(c){
		case "f":
			if(current.forwardScen!=null) current= current.forwardScen;
			else current = new Scenario(0,1,'f',current);
			break;
		case "b":
			if(current.backScen!=null) current= current.backScen;
			else current = new Scenario(0,1,'b',current);
			break;
		case "l":
			if(current.leftScen!=null) current= current.leftScen;
			else current = new Scenario(0,1,'l',current);
			break;
		case "r":
			if(current.rightScen!=null) current= current.rightScen;
			else current = new Scenario(0,1,'r',current);
			break;
		}
	}
